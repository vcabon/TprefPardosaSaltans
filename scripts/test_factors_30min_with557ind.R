...
# format Tpref data set to keep only data from the last 60 minutes and transpose individual Tpref data to columns

d_temp_30min =as.data.frame(t(test1_ID95[,c(1, 39:(ncol(test1_ID95)-1))]))
...
# create empty data frame to compile mean and SD values from fitted gaussian distributions  
sortie2=data.frame('Identifiant'=rep(0,1), 'mean'=rep(0,1), 'SD'=rep(0,1))

# Fit gaussian distribution to each individual (column) and save results (mean and SD) in the table "sortie2"
...
DATA_complet_30min_test <- test1_ID95[, -c(10:(ncol(test1_ID95)-1))] # set base table and remove single Tpref measurements 
DATA_complet_30min_test <- inner_join(DATA_complet_30min_test, sortie2, by="Identifiant") # merge mean and SD values  
DATA_PARDOSES_30min_test <- inner_join(DATA_complet_30min_test, d_indiv, by = "Identifiant") # merge individuals information
...
DATA_PARDOSES_30min_test <- inner_join(DATA_PARDOSES_30min_test, length_cephalothorax, by = "Identifiant") # Merge morphological data (body size)
DATA_PARDOSES_30min_test <- inner_join(DATA_PARDOSES_30min_test, length_legs, by = "Identifiant") # Merge morphological data (legs' length)
DATA_PARDOSES_30min_test <- DATA_PARDOSES_30min_test[!duplicated(DATA_PARDOSES_30min_test$Identifiant), ] # remove duplicates 

### create separate data sets for adults and juveniles

DATA_PARDOSES_30min_test_adults <- filter(DATA_PARDOSES_30min_test, DATA_PARDOSES_30min_test$Dev_stage=="Adult")
DATA_PARDOSES_30min_test_juveniles <- filter(DATA_PARDOSES_30min_test, DATA_PARDOSES_30min_test$Dev_stage=="Juvenile")

### create separate data sets for males and females

DATA_PARDOSES_30min_test_males <- filter(DATA_PARDOSES_30min_test, DATA_PARDOSES_30min_test$Sex=="M")
DATA_PARDOSES_30min_test_females <- filter(DATA_PARDOSES_30min_test, DATA_PARDOSES_30min_test$Sex=="F")
...
## run models on median Tpref

res_Tpref_median_all <- run_models_factors_Tpref(DATA_PARDOSES_30min_test, 
                                                DATA_PARDOSES_30min_test$median, 
                                                DATA_PARDOSES_30min_test$Dev_stage.x) # all individuals
write.csv(rbind.fill(res_Tpref_median_all), "./outputs/res_Tpref_median_all.csv")

res_Tpref_median_adults <- run_models_factors_Tpref(DATA_PARDOSES_30min_test_adults, 
                                                 DATA_PARDOSES_30min_test_adults$median) # only for adults
write.csv(rbind.fill(res_Tpref_median_adults), "./outputs/res_Tpref_median_adults.csv")

res_Tpref_median_juveniles <- run_models_factors_Tpref(DATA_PARDOSES_30min_test_juveniles, 
                                                       DATA_PARDOSES_30min_test_juveniles$median) # only for juveniles
write.csv(rbind.fill(res_Tpref_median_juveniles), "./outputs/res_Tpref_median_juveniles.csv")
